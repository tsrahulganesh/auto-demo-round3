...
public class Login_invalidTest {
    ...
    private void waitForDocumentReady(WebDriver driver) {
        wait(driver, 15).until((ExpectedCondition<Boolean>) d -> ((JavascriptExecutor) d)
                .executeScript("return document.readyState").equals("complete"));
    }

    /**
     * Try default content; if inputs aren't found, iterate iframes and switch into
     * the first one that contains the username field.
     */
    ...
    private void performLogin(WebDriver driver, String user, String pass) {
        ...
        try {
            wait(driver, 5).until((ExpectedCondition<Boolean>) d -> ((JavascriptExecutor) d)
                    .executeScript("return document.readyState").equals("complete"));
